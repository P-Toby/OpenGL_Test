...
int texturePractice()
{
	...
	glBindTexture(GL_TEXTURE_2D, texture); // All upcoming GL_TEXTURE_2D functions now affect this texture
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform1i(glGetUniformLocation(shaderProgram, "ourTexture1"), 0); //Texture unit 0 always active so this is not really needed but made for the sake of clarity
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, texture2);
